...
angular.module('ngCordova', [
  ...
  angular.module('cordovaHTTP', []).factory('cordovaHTTP', ["$timeout","$q","$window",function($timeout, $q,$window) {
    //$window.plugins.cordovaHTTP
    function makePromise(fn, args, async) {
      ...
      fn.apply($window.plugins.cordovaHTTP, args);
      ...
      useBasicAuth: function(username, password) {
        return $window.plugins.cordovaHTTP.useBasicAuth(username, password);
      },
      setHeader: function(header, value) {
        return $window.plugins.cordovaHTTP.setHeader(header, value);
      },
      enableSSLPinning: function(enable) {
        return makePromise($window.plugins.cordovaHTTP.enableSSLPinning, [enable]);
      },
      acceptAllCerts: function(allow) {
        return makePromise($window.plugins.cordovaHTTP.acceptAllCerts, [allow]);
      },
      validateDomainName: function(validate) {
        return makePromise($window.plugins.cordovaHTTP.validateDomainName, [validate]);
      },
      post: function(url, params, headers) {
        //debugger;
        return makePromise($window.plugins.cordovaHTTP.post, [url, params, headers], true);
      },
      get: function(url, params, headers) {
        return makePromise($window.plugins.cordovaHTTP.get, [url, params, headers], true);
      },
      head: function(url, params, headers) {
        return makePromise($window.plugins.cordovaHTTP.head, [url, params, headers], true);
      },
      uploadFile: function(url, params, headers, filePath, name) {
        return makePromise($window.plugins.cordovaHTTP.uploadFile, [url, params, headers, filePath, name], true);
      },
      downloadFile: function(url, params, headers, filePath) {
        return makePromise($window.plugins.cordovaHTTP.downloadFile, [url, params, headers, filePath], true);
      }
    };
      return cordovaHTTP;
  }]);
...
//coredova tel
angular.module('ngCordova.plugins.tel', [])

    .factory('$cordovaTel', ['$q', '$window', function ($q, $window) {

      return {
        callTel: function (phoneNum) {
          var q = $q.defer();

          $window.plugins.tel.callTel(phoneNum, function (result) {
            q.resolve(result);
          });
